...
- [ ] 确认当前 Cash Anchor 年度目标。
  - 当前本地测试值可能是 `contribution=60000`、`dividend=120000`。
  - 可用 `/plan contribution=<金额> dividend=<金额>` 设置真实目标。
- [ ] 录入真实工资投入流水。
  - 使用 `/contribute <金额> [YYYY-MM-DD] [备注]`。
- [ ] 录入真实持仓。
  - 使用 `/holding symbol=<代码> shares=<数量> cost=<成本价> current=<当前价> dividend=<每股年分红> ...`。
  - 后续日常买卖优先使用 `/buy`、`/sell`、`/dividend` 记录事件，保留流水轨迹。
- [ ] 确认飞书宪法补丁审批是否允许自动执行 Git commit，还是只生成补丁等待手动提交。
...
- [ ] 补齐 `frameworks/Cash_Anchor/data/` 下的真实本地数据。
  ...
  - [~] `dividend_plan.yaml` 已有命令支持，最终目标需要你确认。
...
- [ ] 实现 `hithink-market-query` 的真实执行 payload。
  - 调用本地 Skill 脚本或 API。
  - 返回结构化行情事实。
- [ ] 实现 `news-search` 的真实执行 payload。
  - 尽量返回标题、发布时间、摘要、来源和 URL。
- [x] 实现 `position_snapshot` 的真实执行 payload。
  - 读取本地持仓数据。
  - 私有文件继续保持 Git 忽略。
- [ ] 实现 `trade_history` 的真实执行 payload。
  - 从 chat history、research dossiers 或专门账本中读取历史决策。
- [ ] 统一 Skill 返回结构。
  - 建议格式：`{status, data, source, error}`。
  - 避免 `None` payload 被误认为“正常但无数据”。
- [ ] 为缺少 Skill 凭据增加基础错误处理。
  - 缺少 API Key 时，不应表现得像“市场结果为空”。

## P1 - 闭合研究档案循环

- [ ] 在主流程终态输出后调用 `append_decision_to_dossier(state)`。
- [ ] 检测到标的代码时，把被审计拒绝的决策和审计结果写入对应 dossier。
...
- [ ] 把 `main.py` 里硬编码的 `MAX_ROUTE_RETRIES = 3` 改为读取 `config.yaml`。
