...
4. 本地留痕：自动化输出写入 `reports/`，并归档到 `chat_history/`、`runtime/decisions/`、`runtime/traces/`。
5. 输出边界：只给建议和待确认事项；涉及框架修改时只提出建议，不直接修改 `constitution.md`。
...
flowchart TD
  ...
  H --> I["拉取行情、新闻、公告"]
  I --> J["读取研究档案摘要"]
  J --> K["拼装 scheduled_review context"]
  K --> L["LLM 生成开盘前计划"]
  L --> M["写入 reports/daily_reviews"]
  M --> N["归档 chat_history / decisions / traces"]
  N --> O["communication_gate 推送"]
```
