...
6. 用户可用 slash command 手动查看持仓、长桥同步结果、Growth universe、研究档案和定时任务。
...
flowchart LR
  ...
  B["longbridge positions/watchlist"] --> D["growth_universe"]
  D --> C
  E["market_data 行情"] --> C
  F["market_intel 新闻公告"] --> C
...
| Growth universe | 长桥只读数据，经 `src/growth_universe.py` 归一化 |
...
3. Growth universe 依赖本机 `longbridge positions` 和 `longbridge watchlist` 可用。
4. 长桥读取失败时，Growth 自动化会记录数据缺口，不回退到手工名单。
