...
| `/longbridge-account` | 读取长桥账户资产、组合、历史订单和历史成交只读快照 |
| `/longbridge-market` | 读取长桥行情、K 线、市场状态、交易时段、交易日历和市场温度只读快照 |
| `/longbridge-fundamental` | 读取长桥公司概况、估值、财报速览、EPS 预测、一致预期和分红历史只读快照 |
| `/longbridge-events` | 读取长桥新闻、监管披露、社区话题和财经日历只读快照 |
| `/longbridge-options` | 读取长桥期权链、到期日、合约 quote、成交量和 put/call 风险只读快照 |
...
| 长桥账户/成交快照 | `scheduled_review context.account_activity`，由 `src/longbridge_account_provider.py` 生成并压缩 |
| 长桥行情快照 | `scheduled_review context.longbridge_market_context`，由 `src/longbridge_quote_provider.py` 生成并压缩 |
| 长桥基本面快照 | `scheduled_review context.longbridge_fundamental_context`，由 `src/longbridge_fundamental_provider.py` 生成并压缩 |
| 长桥资讯事件快照 | `scheduled_review context.longbridge_event_context`，由 `src/longbridge_event_provider.py` 生成并压缩，再合并进 `symbol_intel` |
| 长桥期权快照 | `scheduled_review context.longbridge_options_context`，由 `src/longbridge_options_provider.py` 生成并压缩，再进入 Research Engine 风险证据 |
...
| FR-037 | 长桥基本面进入 Research Engine | 已实现 |
| FR-038 | 长桥订单/成交只读查询进入复盘连续性 | 进行中：已进入 context |
| FR-039 | 长桥期权链进入风险识别 | 已实现 |
| FR-040 | 长桥健康诊断命令 | 已实现 |
| FR-041 | 长桥账户资产、组合、历史订单和成交只读 Provider | 已实现 |
| FR-042 | 长桥行情、K线、市场状态和交易日历只读 Provider | 已实现 |
| FR-043 | 长桥行情上下文进入 scheduled_review 和 Research Engine | 已实现 |
| FR-044 | 长桥公司概况、估值、财报速览和预期只读 Provider | 已实现 |
| FR-045 | 长桥新闻、监管披露、社区话题和财经日历进入 scheduled_review 和 Research Engine | 已实现 |
| FR-046 | 长桥期权链、到期日、合约 quote 和 put/call 风险进入 scheduled_review 和 Research Engine | 已实现 |
...
3. Growth universe 依赖本机 `longbridge positions` 和 `longbridge watchlist` 可用；长桥只读数据平台已经接入账户、行情、基本面、资讯事件和期权，期权行情可能需要 OPRA 权限，更完整交易查询仍在后续扩展中。
