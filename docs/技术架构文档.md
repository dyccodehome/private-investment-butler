...
| 长桥只读数据平台层 | `src/longbridge_provider.py`, `src/longbridge_account_provider.py`, `src/longbridge_quote_provider.py`, `src/longbridge_fundamental_provider.py`, `src/longbridge_event_provider.py`, `src/growth_universe.py` | 券商账户、行情、基本面、资讯、期权和交易查询的只读接入 |
...
当前实现已经接入长桥 CLI 的持仓、自选、账户资产、历史订单、历史成交、报价、K 线、市场状态、交易时段、交易日历、市场温度、公司概况、估值、财报速览、EPS 预测、一致预期、分红历史、新闻、监管披露、社区话题、财经日历和汇率能力。后续继续把长桥扩展为 Longbridge Read-only Data Hub 的期权和更完整执行查询事实源，但所有交易写操作保持禁用。
...
flowchart TD
  ...
  KE --> L["fetch_company_news / announcements"]
