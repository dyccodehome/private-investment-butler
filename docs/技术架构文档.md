...
| `src/longbridge_capabilities.py` | 待实现 | 只读 allowlist 和交易写 denylist |
| `src/longbridge_account_provider.py` | 待实现 | 账户、持仓、现金、资金流水、订单/成交只读查询 |
...
| 长桥账户数据 | `src/longbridge_provider.py`，后续 `src/longbridge_account_provider.py` | 只读读取资产、持仓、现金、资金流水、历史订单和成交 |
