...
| 长桥 universe 层 | `src/longbridge_provider.py`, `src/growth_universe.py` | 券商只读同步、Growth universe 归一化 |
...
| 长桥持仓和自选股 | `src/longbridge_provider.py` | 固定白名单命令，只读同步 |
...
LLM 不允许直接调用 shell、券商 CLI 或外部 API。所有外部数据必须通过固定 Python 函数进入。
